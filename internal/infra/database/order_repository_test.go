...
func (suite *OrderRepositoryTestSuite) AfterTest(suiteName, testName string) {
	suite.Db.Exec("DELETE FROM orders WHERE true")
}
...
func (suite *OrderRepositoryTestSuite) TestGivenAnOrder_WhenSave_ThenShouldQueryOrders() {
	order, err := entity.NewOrder("123", 10.0, 2.0)
	suite.NoError(err)
	suite.NoError(order.CalculateFinalPrice())
	...
	fromDb, err := repo.GetAll()
	suite.NoError(err)
	suite.Equal(len(fromDb), 2)
}
